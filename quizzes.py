...
def quiz(questions, options, answers, topic_choice, username):
    try:
        ...
        # Zip questions, options and answers so that random treats their index
        # positons as the same when shuffling
        ...
        for question in shuffled_questions:
            ...
            if guess == shuffled_answers[question_num]:
                ...
                print(
                    f"[bold green]{
                        shuffled_answers[question_num]} is the correct answer.[/bold green]")
            question_num += 1
        ...
        print("[purple]-----------------------------[purple]")
        print(
            "       [bold bright_red]   RESULTS       [/bold bright_red]     ")
        print("[purple]-----------------------------[purple]\n")
        ...
        total_points = calculate_points(
            score, len(shuffled_questions), total_time)
